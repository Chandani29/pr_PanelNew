...
{  
		...
		{
			//alert(var1);
			window.location = "update_inhouse_workstatus.asp?h_id=" + var1 + "&refno=" + var2;						
		}
		else
		{
			//alert(var2);			
			window.location = "inhouse_project_report.asp?refno=" + var2;
		}
}
...
'						end if	
	...
	%>
			<table width="40%" border="1" cellpadding="3" cellspacing="1" class="Tab2" align="center">
				<tr valign="top" bgcolor="#E6E6E6" class="tdrow3" align="center"> 
	...
	<%
					do while not rst_inhouse.eof 
						...
						subproj_sql = "select sum(hourspend) as hourspend_inhouse,sum(dev_cost) as dev_cost_inhouse from tbl_pr_projDetails where inhouse_id='" & trim(rst_inhouse("srno")) & "' and working_per='" & trim(Session("DeveloperName")) & "' and (work_by_mark is null or work_by_mark ='') and del_status='0'"
...
'						end if 
      ...
      <tr valign="top" bgcolor="#E6E6E6" class="tb2" > 
		...
		%>
		<td class='<%=class1%>'><%=rsInhouse("ProjName")%>&nbsp;</td>
		<% end if %>
	...
	%>
        <td class='<%=class1%>'><font color="#FF0000">Pending&nbsp;&nbsp;<a href="inhouse_task_reply.asp?h_id=<%=rsDeveloper("srno")%>&refno=<%=refno%>&sub_id=<%=rsDeveloper("inhouse_id")%>">Reply</a>&nbsp;&nbsp;<a href="#" onClick="confirmform(<%=rsDeveloper("srno")%>,<%=refno%>);"><font color="#FF00FF">Done</font></a></font></td>
        <%
							else
	%>
        ...
        <%					
				...
				if isnull(rsDeveloper("work_by_mark")) or rsDeveloper("work_by_mark")="" then
					if(trim(rsDeveloper("working_per")) = trim(Session("DeveloperName"))) then
						...
						%>
        <td class='<%=class1%>'><a href="inhouse_deleted_resign.asp?h_id=<%=rsDeveloper("srno")%>&refno=<%=refno%>"><font color="#FF0000">Delete</font></a></td>
        <%
						else
	%>
...
'						end if
      ...
      <tr valign="top" bgcolor="#E6E6E6" class="tb2" > 
		...
		<%
			set rsInhouse = server.CreateObject("adodb.recordset")
			if rsInhouse.state then rsDeveloper1.close
	...
	%>
        <td class='<%=class1%>'><font color="#FF0000">Pending&nbsp;&nbsp;<a href="inhouse_task_reply.asp?h_id=<%=rsDeveloper1("srno")%>&refno=<%=refno%>&sub_id=<%=rsDeveloper1("inhouse_id")%>">Reply</a>&nbsp;&nbsp;<a href="#" onClick="confirmform(<%=rsDeveloper1("srno")%>,<%=refno%>);"><font color="#FF00FF">Done</font></a></font></td>
        <%
							else
	%>
        <td class='<%=class1%>'>Pending</td>
        ...
        <%					
				...
				if isnull(rsDeveloper1("work_by_mark")) or rsDeveloper1("work_by_mark")="" then
					if(trim(rsDeveloper1("working_per")) = trim(Session("DeveloperName"))) then
						...
						%>
        <td class='<%=class1%>'><a href="inhouse_deleted_resign.asp?h_id=<%=rsDeveloper1("srno")%>&refno=<%=refno%>"><font color="#FF0000">Delete</font></a></td>
        <%
						else
	%>
