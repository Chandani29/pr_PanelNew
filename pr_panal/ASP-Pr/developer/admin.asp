...
<%
	...
	If Len(	Username ) > 0 Then
	  call OpenConnection
		...
		Set rsUser = NewConnection.Execute("SELECT user_id,name FROM tbl_pr_developerLogin WHERE user_id='" & Username & "'AND user_pass='" & Password & "'")
		...
		Else
			...
			Session("dr_username") = rsUser("user_id")
			Session("DeveloperName") = rsUser("name")
			Session("adminlogin") = "yes"
			response.write Session("dr_username")			
			Response.redirect "developer_assign_proj.asp"
			'response.end
			rsUser.close
