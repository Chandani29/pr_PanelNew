...
if Session("pr_username")="" then
	...
	'response.Write(txt_dateFrom1)
	'response.Write(txt_dateTo1)
dim ddate
ddate = now()
%>
<html>
...
<table width="70%" border="1" cellpadding="3" cellspacing="1" class="Tab2" align="center">
  <tr valign="top" bgcolor="#E6E6E6" class="tdrow3" align="center"> 
    <td colspan="14" class="tdrow5" bgcolor="#CCCCCC"><div align="center"><strong>All 
        Activities ::&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;From::<%=txt_dateFrom%>&nbsp;&nbsp;&nbsp;&nbsp;To::<%=txt_dateTo%></strong></div></td>
  </tr>
  <%
			call OpenConnection
			'refno = request("refno") 
			dim asignedby
			asignedby = trim(Session("MarketingName"))
			dim rsDeveloper1
			set rsDeveloper1 = server.CreateObject("adodb.recordset")			
				if rsDeveloper1.state then rsDeveloper1.close
				rsDeveloper1.open "SELECT * from tbl_pr_projDetails where asignedby='" & asignedby & "' and ddate between '" & txt_dateFrom1 & "' and '" & txt_dateTo1 & "' order by srno desc",NewConnection
...
'						end if
  ...
  <tr valign="top" bgcolor="#E6E6E6" class="tb2" > 
    ...
    <td class='<%=class1%>'><%=ddate_india2%>&nbsp;</td>
	...
	%>
	<td class='<%=class1%>'><%=rsDeveloper1("proj_name")%><br>(<%=rsInhouse("ProjName")%>)&nbsp;</td>
	<% end if %>	
    ...
    <%
			...
			%>
    <!--td class='<%'=class1%>'><%'=rsDeveloper1("work_by_mark")%>&nbsp;</td-->
    <td class='<%=class1%>'><%=rsDeveloper1("hourspend")%>&nbsp;</td>
